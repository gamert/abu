...
from abupy import AbuPickTimeWorker, AbuFactorBuyBreak, AbuFactorSellBreak, AbuFactorPreAtrNStop, \
    AbuFactorCloseAtrNStop, AbuFactorAtrNStop, AbuBenchmark, AbuCapital, EMarketTargetType, AbuPickTimeWorkBase, \
    AbuMulPidProgress, AbuFactorBuyBase, AbuFactorSellBase
from abupy.AlphaBu.ABuPickTimeWorker import g_natural_long_task
...
class QAStrategyPickTime(QAStrategyStockBase,AbuPickTimeWorkBase):

    # 初始化AbuPickTimeWorkBase
    def initPickTimeWorker(self, cap, benchmark, buy_factors, sell_factors):
        """
        ...
        self.capital = cap
        # 回测阶段kl
        self.kl_pd = self._market_data  #
        # 合并加上回测之前1年的数据，为了生成特征数据
        self.combine_kl_pd = self.kl_pd;## ABuSymbolPd.combine_pre_kl_pd(self.kl_pd, n_folds=1)
    ...
    def fit(self, *args, **kwargs):
        ...
        self.kl_pd.apply(self._task_loop_day, axis=1)
    ...
    def on_bar(self, data):
        ...
        # 2. 再取最后一行..
        #
        new_data = self._market_data[-1];
        self._task_loop_day(new_data)
        # print(data)
        ...
        # print(code)
        # self.pick_timer_worker._task_loop_day(data)
